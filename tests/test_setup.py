"""
Unit tests for panda.setup — interactive configuration wizard.
...
Tests cover:
  ...
  - CLI integration (panda setup --help, panda setup --quick)
"""
...
from panda.setup import test_feishu as _test_feishu_connectivity
...
class TestSetupDefaults:
    def test_returns_all_defaults(self):
        with patch("builtins.input", side_effect=["gpt-4o", "openai", "8000"]):
            ...
            assert result["PANDA_DEFAULT_MODEL"] == "gpt-4o"
            assert result["PANDA_DEFAULT_PROVIDER"] == "openai"
            assert result["PANDA_SERVER_PORT"] == "8000"

    def test_custom_values(self):
        with patch("builtins.input", side_effect=["claude-sonnet-4", "anthropic", "3000"]):
            result = setup_defaults()
            assert result["PANDA_DEFAULT_MODEL"] == "claude-sonnet-4"
            assert result["PANDA_DEFAULT_PROVIDER"] == "anthropic"
...
class TestGenerateEnvExample:
    ...
    def test_generates_file(self):
        ...
        with patch.object(__import__("panda.setup", fromlist=["PROJECT_ROOT"]),
                          "PROJECT_ROOT", Path(self.tmpdir)):
            generate_env_example()
            assert (Path(self.tmpdir) / ".env.example").exists()

    def test_contains_all_providers(self):
        with patch("panda.setup.PROJECT_ROOT", Path(self.tmpdir)):
            generate_env_example()
    ...
    def test_contains_defaults(self):
        with patch("panda.setup.PROJECT_ROOT", Path(self.tmpdir)):
            generate_env_example()
            content = (Path(self.tmpdir) / ".env.example").read_text()
            assert "PANDA_DEFAULT_MODEL" in content
            assert "PANDA_SERVER_PORT" in content
...
class TestSetupCLIIntegration:
    def test_setup_help(self):
        result = subprocess.run(
            [sys.executable, "-m", "panda", "setup", "--help"],
    ...
    def test_setup_quick_runs_without_error(self):
        ...
        with _tmp.TemporaryDirectory() as td:
            result = subprocess.run(
                [sys.executable, "-m", "panda", "setup", "--quick"],
                capture_output=True, text=True, timeout=10,
                cwd=td,
                env={**os.environ, "PYTHONPATH": os.path.dirname(os.path.dirname(__file__))}
            )
