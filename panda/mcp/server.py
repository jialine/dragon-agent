...
logger = logging.getLogger("panda.mcp.server")

# ────────────────────────────────────────────────────────────────────
# Panda MCPServer — The actual server
# ────────────────────────────────────────────────────────────────────


class PandaMCPServer:
    """MCP Server that exposes Panda's self-evolving skills and tools.
        ...
        if self._skill_engine is None:
            try:
                ...
                self._skill_engine = SkillEngine(
                    skills_dir=os.path.expanduser("~/.panda/skills"),
                    auto_evolve=True,
                )
    ...
    def _ensure_router(self):
        if self._router is None:
            try:
                from panda.router import PandaRouter
                from panda.config import PandaConfig
                cfg = PandaConfig.load()
                self._router = PandaRouter(
    ...
    def _ensure_memory(self):
        if self._memory_graph is None:
            try:
                from panda.memory import MemoryGraph
                from panda.config import PandaConfig
                cfg = PandaConfig.load()
    ...
    async def _handle_tools_list(self, params: Dict) -> Dict:
        ...
        # 1. Panda-specific tools (self-evolving, unique to Panda)
        tools.append({
            "name": "panda.skills.search",
        ...
        tools.append({
            "name": "panda.consult.debate",
            "description": "Multi-model expert consultation. THREE independent models debate the problem, vote, and synthesize a consensus answer. Use when panda.consult.assess returns difficulty >= 7.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        # 2. Standard built-in tools (from Panda's ToolRegistry)
        if self._tool_registry:
    ...
    async def _call_consult_assess(self, args: Dict) -> Dict:
        ...
        return {"content": [{"type": "text", "text": (
            ...
            f"{'👉 Use panda.consult.debate to start expert consultation' if difficulty_score >= 7 else '✅ Safe to proceed with standard tools'}"
        )}]}
    ...
    async def _handle_resources_read(self, params: Dict) -> Dict:
        ...
        if not uri.startswith("panda://skills/"):
            raise ValueError(f"Unknown resource URI: {uri}")

        skill_name = uri.replace("panda://skills/", "")
        self._ensure_skill_engine()
                ...
                "messages": [{
                    ...
                    "content": {
                        ...
                        "text": (
                            ...
                            f"then debating to reach a consensus. Use panda.consult.debate to begin."
                        ),
                    },
                }],
            }
                        ...
                        "messages": [{
                            ...
                            "content": {
                                ...
                                "text": (
                                    ...
                                    f"Use panda.skills.evolve to apply changes."
                                ),
                            },
                        }],
                    }
    ...
    async def run(self) -> None:
        ...
        logger.info("Panda MCP Server starting (v%s)", self.version)
...
def main():
    """Entry point for `python -m panda.mcp.server`."""
    logging.basicConfig(
