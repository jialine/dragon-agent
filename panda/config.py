...
class BackupConfig(BaseModel):
    endpoint: str = ""
    access_key_env: str = "PANDA_BACKUP_ACCESS_KEY"
    secret_key_env: str = "PANDA_BACKUP_SECRET_KEY"
...
class PandaConfig(BaseModel):
    ...
    @classmethod
    def load(cls, config_path: str = "config.yaml") -> "PandaConfig":
...
# Known PandaConfig section names (must match PandaConfig model fields)
_PANDA_SECTIONS = frozenset({"router", "dispatch", "memory", "backup", "guard", "server"})


def _apply_env_overrides(data: dict, prefix: str = "PANDA_"):
    """Apply PANDA_* environment variables as overrides."""
    for key, value in os.environ.items():
        if not key.startswith(prefix):
            continue
        # PANDA_BACKUP_INTERVAL_HOURS → backup.interval_hours
        # PANDA_SERVER_PORT → server.port
        ...
        for i in range(1, len(parts)):
            ...
            if candidate in _PANDA_SECTIONS:
                section = candidate
        ...
        if field is None:
            # PANDA_SOMETHING without further qualifiers → data[something] = value
