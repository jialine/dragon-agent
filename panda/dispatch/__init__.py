...
# Structured logging
# ────────────────────────────────────────────────────────────────────
logger = logging.getLogger("panda.dispatch")
...
class PandaDispatcher:
    ...
    * **Thread-safe** — async throughout; uses ``httpx.AsyncClient`` pool
    ...
    def __init__(
        ...
        self._lock = asyncio.Lock()
        logger.info(
            "PandaDispatcher initialized (circuit: %d failures → %.0fs open)",
    ...
    async def close(self) -> None:
        ...
        logger.info("PandaDispatcher closed — all clients shut down")

    # ── Async Context Manager ──────────────────────────────────────────

    async def __aenter__(self) -> "PandaDispatcher":
        return self
