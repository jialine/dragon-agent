#!/usr/bin/env python3
"""
Panda CLI — Command-line interface for Panda Agent.

Usage:
    panda chat              Start interactive chat
    panda serve             Start API server
    panda gateway           Start multi-platform gateway
    panda mcp               Start MCP server
    panda config            Manage configuration (init, validate, show, check)
    panda skills            Manage self-evolving skills
    panda tools             Manage tools
    panda sessions          Manage sessions (list, export, stats, search, get, delete)
    panda cron              Manage scheduled jobs
    panda profile           Manage profiles (list, create, edit, clone, export, import, use, delete)
    panda test              Run tests
    panda doctor            运行诊断检查
    panda version           Show version
"""
...
logger = logging.getLogger("panda.cli")


def main():
    parser = argparse.ArgumentParser(
        prog="panda",
        description="Panda Agent — Self-Evolving AI Agent Framework",
    )
...
def cmd_chat(args):
    ...
    if args.query:
        print(f"[Panda] Processing: {args.query}")
        ...
        print(f"Panda Agent v{VERSION} — Interactive Mode")
...
def cmd_serve(args):
    """Start API server."""
    print(f"Starting Panda API server on {args.host}:{args.port}...")
    try:
        import uvicorn
        uvicorn.run(
            "panda.main:app",
...
def cmd_gateway(args):
    ...
    if args.discord:
        from panda.gateway.discord import DiscordAdapter
        server.register_adapter(DiscordAdapter())
        print("  ✓ Discord enabled")

    if args.wechat:
        from panda.gateway.wechat import WeChatAdapter
...
def _cmd_gateway_status():
    ...
    for env_var in ["OPENAI_API_KEY", "DEEPSEEK_API_KEY", "ANTHROPIC_API_KEY",
                     "PANDA_GENERAL_API_KEY", "OPENROUTER_API_KEY"]:
...
def cmd_mcp(args):
    ...
    if args.action == "tools":
        print("Panda MCP Tools:")
        print("  panda.skills.search")
        print("  panda.skills.evolve")
        print("  panda.consult.assess")
        print("  panda.consult.debate")
        print("  panda.memory.search")
        print("  panda.memory.graph")
        print("  panda.search")
        print("  panda.file_read")
        print("  panda.file_write")
        print("  panda.execute")
        print("  panda.http_get")
    else:
        print("Starting Panda MCP Server...")
        from panda.mcp.server import main as mcp_main
        mcp_main()
...
def cmd_config(args):
    ...
    if args.action == "show":
        ...
        print("config.yaml: ~/.panda/config.yaml")
        print(".env:        ~/.panda/.env")
...
def _cmd_config_init(args):
    ...
    print("Panda Config Init — 交互式配置向导")
    print("=" * 40)
    print("按 Enter 使用默认值。\n")
...
def _cmd_config_validate():
    """Validate config.yaml against the PandaConfig schema."""
    from panda.config import PandaConfig

    config_path = "config.yaml"
    if not Path(config_path).exists():
        print(f"⚠ 未找到配置文件: {config_path}")
        config_path = os.path.expanduser("~/.panda/config.yaml")
        if not Path(config_path).exists():
            print("⚠ 未找到配置文件: ~/.panda/config.yaml")
            print("请运行: panda config init")
            return

    print(f"验证配置文件: {config_path}")
    try:
        cfg = PandaConfig.load(config_path)
        print("✓ 配置文件格式正确")
...
def cmd_skills(args):
    ...
    if args.action == "list":
        ...
        if not args.name or not args.content:
            print("Usage: panda skills create <name> --content <text> --description <desc>")
            return
        engine.register(
            name=args.name,
...
def cmd_tools(args):
    ...
    if args.action == "list":
        ...
        if not args.name:
            print("Usage: panda tools call <name> --args '{\"key\":\"value\"}'")
            return
        try:
...
def cmd_sessions(args):
    ...
    if args.action == "list":
        ...
        if not args.session_id:
            print("Usage: panda sessions get <session_id>")
            return
        ...
        if not args.session_id:
            print("Usage: panda sessions delete <session_id>")
            return
...
def _cmd_sessions_export(args, store):
    ...
    if not args.session_id:
        print("Usage: panda sessions export <session_id> [--output <file>]")
        return
...
def cmd_cron(args):
    ...
    if args.action == "list":
        ...
        if not args.name or not args.schedule:
            print("Usage: panda cron add --name <name> --schedule <30m|0 9 * * *> --task <desc>")
            return
...
def cmd_profile(args):
    ...
    if args.action == "list":
        ...
        if not args.name:
            print("Usage: panda profile create <name> [--clone <source>]")
            return
        ...
        if not args.name or not args.target:
            print("Usage: panda profile rename <old_name> <new_name>")
            return
        ...
        if not args.name:
            print("Usage: panda profile use <name>")
            return
        ...
        if not args.name:
            print("Usage: panda profile delete <name>")
            return
...
def _cmd_profile_clone(args, pm):
    """Clone a profile: panda profile clone <source> <target>."""
    if not args.name:
        print("Usage: panda profile clone <source_profile> <new_name>")
        return
...
def _cmd_profile_export(args, pm):
    ...
    if not args.name:
        print("Usage: panda profile export <name> [--output <path>]")
        return
...
def _cmd_profile_import(args, pm):
    ...
    if not args.name:
        print("Usage: panda profile import <archive.tar.gz> [new_name]")
        return
