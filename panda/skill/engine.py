"""
Panda Agent — Skill Engine (Semantic Discovery + Self-Evolution + Pipelines)
============================================================================

The SkillEngine is the central orchestrator for Panda's skill system.
It provides:
...
Usage::
    ...
    engine = SkillEngine(skills_dir="~/.panda/skills/")
...
logger = logging.getLogger("panda.skill.engine")

# ────────────────────────────────────────────────────────────────────
# Constants
# ────────────────────────────────────────────────────────────────────

DEFAULT_SKILLS_DIR = "~/.panda/skills/"
DEFAULT_VECTORDB_DIR = "panda_data/skill_vectors"
...
class SkillPipeline:
    """Compose multiple PandaSkills into a workflow.
...
class SkillEngine:
    """Central orchestrator for Panda's self-evolving skill system.
    ...
    def __init__(
        ...
        # Skills registry: name → PandaSkill
        self._skills: Dict[str, PandaSkill] = OrderedDict()
        ...
        # Execution callback — set by external system (e.g., PandaDispatcher)
    ...
    def register_executor(self, executor: Callable) -> None:
        ...
        The executor receives (PandaSkill, context_dict) and returns
    ...
    def register(
        ...
        with self._lock:
            ...
            skill = PandaSkill(meta=meta, content=content)
            self._skills[name] = skill
            self._persist_skill(skill)
            logger.info("Registered skill: %s (v%s)", name, version)
            return skill

    def get(self, name: str) -> Optional[PandaSkill]:
        """Get a skill by name."""
    ...
    def _keyword_discover(self, query: str, top_k: int) -> List[SkillMatch]:
        ...
        scored: List[Tuple[float, PandaSkill]] = []
    ...
    def _check_evolution(self, skill: PandaSkill) -> None:
    ...
    def _persist_skill(self, skill: PandaSkill) -> None:
