#!/usr/bin/env python3
"""
Dragon TUI JSON-RPC Backend Server
=================================
...
Usage:
    python -m dragon.tui.server          # in $PATH / from dragon CLI
...
logger = logging.getLogger("dragon.tui")

# ────────────────────────────────────────────────────────────────────
# Lazy imports — only load dragon modules when actually needed
# ────────────────────────────────────────────────────────────────────
...
async def handle_config_get(params: Dict) -> Dict:
    """Get current Dragon configuration."""
    cfg = _get_config()
    return {
        "router": {
...
async def handle_doctor(params: Dict) -> Dict:
    """Run diagnostic checks (like dragon doctor CLI)."""
    import importlib
...
async def run_server() -> None:
    ...
    logger.info("Dragon TUI Server v%s starting on stdin/stdout", VERSION)
    await _read_stdin()
    logger.info("Dragon TUI Server shutting down")


def main() -> None:
    """Entry point for `dragon tui` CLI command."""
    try:
