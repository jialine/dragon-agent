"""
Dragon Agent Backup Module — full backup + restore.
...
logger = logging.getLogger("dragon.backup")
...
class DragonBackup:
    """
    Cloud backup + restore for Dragon Agent data.
    
    Backs up:
      - ChromaDB (dragon_data/vectordb/)
      - Memory graph (dragon_data/memory/graph.json)
      - Config (config.yaml)
    ...
    Supports:
      ...
      - Restore to dragon_data/restored/ (never overwrites live data)
    """
    ...
    def backup(self) -> BackupManifest:
        ...
        try:
            ...
            tmpdir = tempfile.mkdtemp(prefix="dragon_backup_")
            ...
            backup_id = f"dragon_backup_{timestamp}"
    ...
    def restore(self, backup_id: Optional[str] = None) -> BackupManifest:
        ...
        IMPORTANT: Restores to dragon_data/restored/ — NEVER overwrites live data.
        """
        # 1. Find backup
        if backup_id:
            key = f"{self.prefix}{backup_id}.tar.gz"
        ...
        tmpdir = tempfile.mkdtemp(prefix="dragon_restore_")
        ...
        try:
            ...
            logger.info(
                ...
                "  WARNING: Data extracted to dragon_data/restored/ — NOT live.\n"
                "  To activate: stop Dragon, move dragon_data/restored/* → dragon_data/, restart.",
        ...
        try:
            ...
            for obj in contents:
                ...
                if not key.endswith(".tar.gz"):
                    continue
                # Extract backup_id from key: dragon/backups/dragon_backup_20260519_120000.tar.gz
                filename = key.rsplit("/", 1)[-1]
                backup_id = filename.replace(".tar.gz", "")
                timestamp_str = backup_id.replace("dragon_backup_", "")
                try:
    ...
    @classmethod
    def from_config(cls, config) -> "DragonBackup":
        """Create DragonBackup from DragonConfig object."""
        import os as _os
        return cls(
