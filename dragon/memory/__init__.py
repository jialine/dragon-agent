...
Storage backends:
  ...
  - Embedding model: BAAI/bge-small-zh-v1.5 (configurable via DragonConfig)

Thread safety: ChromaDB write operations are guarded by ``threading.Lock``.

This module does NOT import ``dragon.router`` at module level to avoid circular
...
logger = logging.getLogger("dragon.memory")
...
class MemoryGraph:
    ...
    Typical usage::
        ...
        memory = MemoryGraph(
            ...
            graph_path="dragon_data/memory/graph.json",
        )
    ...
    _COLLECTION_NAME = "dragon_memory_entities"
    ...
    def __init__(
        ...
        *,
        ...
        graph_path: str = "dragon_data/memory/graph.json",
        ...
        Args:
            ...
            router:           Optional DragonRouter reference for entity extraction.
    ...
    def extract_from_conversation(
        ...
        This method performs a **lazy import** of ``dragon.router`` to avoid
        ...
        from dragon.router import get_router, RouterStatus
...
# ---------------------------------------------------------------------------
# Convenience: create from DragonConfig
# ---------------------------------------------------------------------------
...
def create_memory_from_config(
    ...
    *,
    ...
    """Factory that creates a ``MemoryGraph`` from a ``DragonConfig`` object.

    Args:
        config:     A ``DragonConfig`` instance.  If ``None``, loads from default path.
        graph_path: Override the graph JSON path.
        router:     Optional DragonRouter for entity extraction.
    ...
    memory = MemoryGraph(
        ...
        graph_path=graph_path or "dragon_data/memory/graph.json",
        router=router,
    )
    memory.initialize()
    return memory
