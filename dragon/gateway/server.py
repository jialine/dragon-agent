    ...
    def _build_system_prompt(self) -> str:
        ...
        if base_prompt:
            ...
            prompts.extend([
                ...
                "### 专用工具优先（铁律）",
                "",
                "以下任务已有专用工具，**禁止**自己写代码实现：",
                "- **创建 PDF（含中文）** → 只准用 `pdf_create(path, content, title)`，不准用 execute_code 写 Python 脚本生成 PDF，不准用 terminal 调 fpdf2/reportlab。",
                "- **发送文件/图片** → 用 `send_message` 的 file_paths 参数，不准写 curl 命令或 HTTP 请求。",
                "- **文本转语音** → 用 `text_to_speech`，不准调 edge-tts 命令行。",
                "",
                "当你准备写代码前，先检查：你要做的事是否已有专用工具？如果有，直接调工具，不要写一行代码。",
                "",
                "### 记忆管理",
                "",
