    ...
    def _build_system_prompt(self) -> str:
        ...
        if base_prompt:
            ...
            prompts.extend([
                ...
                "",
                "宁可多加载 — 技能包含专用知识和已验证的工作流（API endpoints、工具调用、陷阱处理），远胜凭通用能力临场发挥。**即使你认为自己能用 terminal 或 execute_code 等基础工具处理，也要先加载技能。** 技能定义了这件事在这里该怎么做，加载了才知道有没有坑。",
                "",
    ...
    def _rebuild_system_prompt_with_memory(self, memory_text: str, user_text: str) -> str:
        ...
        if not base_prompt:
            base_prompt = "\n".join([
                ...
                "",
                "### PDF 创建",
                "用 pdf_create 直接生成——支持标题、粗体、斜体、代码、列表、分页符(---)，零依赖。",
                "正确方式：① write_file 写 .md → ② pdf_create 生成 PDF → ③ MEDIA:xxx.pdf 交付。",
                "禁止用 execute_code 或 Python 代码手搓 PDF——字体嵌入会乱码。",
                "",
                "### 记忆管理",
